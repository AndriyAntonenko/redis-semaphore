...
const debug = (0, debug_1.default)('redis-semaphore:redlock-multi-semaphore:acquire');
async function acquireRedlockMultiSemaphore(clients, key, limit, permits, options) {
    ...
    while ((now = Date.now()) < end && ++attempt <= acquireAttemptsLimit) {
        ...
        const promises = clients.map(client => (0, lua_1.acquireLua)(client, [key, limit, permits, identifier, lockTimeout, now])
            .then(result => +result)
            .catch(() => 0));
