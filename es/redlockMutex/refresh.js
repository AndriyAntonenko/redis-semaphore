...
export async function refreshRedlockMutex(clients, key, identifier, lockTimeout) {
    debug(key, identifier);
    const quorum = getQuorum(clients.length);
    const promises = clients.map(client => expireIfEqualLua(client, [key, identifier, lockTimeout])
        .then(result => +result)
        .catch(() => 0));
