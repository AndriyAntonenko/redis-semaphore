...
export async function acquireRedlockMultiSemaphore(clients, key, limit, permits, options) {
    ...
    while ((now = Date.now()) < end && ++attempt <= acquireAttemptsLimit) {
        ...
        const promises = clients.map(client => acquireLua(client, [key, limit, permits, identifier, lockTimeout, now])
            .then(result => +result)
            .catch(() => 0));
